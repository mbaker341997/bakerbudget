...
const addTransactionToBudget = (transaction, budget) => {
  ...
  return {
    ...
    transactions: [
      ...budget.transactions,
      { ...transaction, categoryName: cat.title, isExpense: cat.isExpense },
    ],
    categories: newCategories,
    incomeTotal: cat.isExpense
      ? budget.incomeTotal
      : budget.incomeTotal + transaction.amount,
    expenseTotal: cat.isExpense
      ? budget.expenseTotal + transaction.amount
      : budget.expenseTotal,
  };
};
...
const removeTransactionFromBudget = (transactionId, budget) => {
  ...
  return {
    ...
    transactions: budget.transactions.filter(
      (budgetTrans) => budgetTrans._id !== transactionId
    ),
    categories: newCategories,
    incomeTotal: cat.isExpense
      ? budget.incomeTotal
...
const addCategoryToBudget = (category, budget) => {
  return {
    ...
    incomeTarget: category.isExpense
      ? budget.incomeTarget
      : budget.incomeTarget + category.target,
    expenseTarget: category.isExpense
      ? budget.expenseTarget + category.target
      : budget.expenseTarget,
  };
};

const removeCategoryFromBudget = (categoryId, budget) => {
  const deadCat = budget.categories.find(
  ...
  return {
    ...
    incomeTarget: deadCat.isExpense
      ? budget.incomeTarget
      : budget.incomeTarget - deadCat.target,
    expenseTarget: deadCat.isExpense
      ? budget.expenseTarget - deadCat.target
      : budget.expenseTarget,
  };
};
